...
public class ChessClient extends Application {
	...
	public void start(Stage primaryStage) {
		...
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new AlphaBetaAlgorithm());
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new AlphaBetaAlgorithm());
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new MiniMaxAlgorithm());
		moveMaker[Chess.WHITE] = new AIMoveMaker(new MiniMaxAlgorithm());
			...
			if (mover.getState() == Worker.State.READY) {
				...
				if(game.position.isMate())
				{
					if(game.position.getToPlay()==0)
						log("Check Mate Game over : Black won the game");
					else
						log("Check Mate Game over : White won the game.");
				}
				
				else if(game.position.isStaleMate() || game.position.isTerminal())
				{
					log("Game cannot move further: Game has been drawn.");
				}
				mover.reset();
			}
